...
class Calculated:
    ...
    def CalculatedPlot(self):
        ...
        plt.scatter(1.4*self.size,1.4*self.size,c='w',marker='o', s=6*self.size,edgecolors='w',linewidths=0.3*self.size, zorder=0.5)      #通过画白色点的方式微调画布尺寸使边角上的力能显示在画布范围内
        plt.scatter(self.x_min-0.4*self.size,self.y_min-0.4*self.size,c='w',marker='o', s=6*self.size,edgecolors='w',linewidths=0.3*self.size, zorder=0.5)      #通过画白色点的方式微调画布尺寸使边角上的力能显示在画布范围内
