...
class TrussAnalyzer(wx.Frame):                        #主界面
        ...
        def __init__(self, parent, id):
                ...
                self.deleteButton = wx.Button(self.panel, wx.NewId(), "DELETE", pos=(270, 170))
                self.Bind(wx.EVT_BUTTON, self.OnDelete, self.deleteButton)
                self.deleteButton.Bind(wx.EVT_ENTER_WINDOW, self.deleteButtonhelp)
                self.deleteButton.Bind(wx.EVT_LEAVE_WINDOW, self.Leave)
        ...
        def OnDelete(self,event):
            ...
            if i == 1 :
                ...
                [a,b]= x.shape
                x = numpy.delete(x, a-1, axis = 0)
                numpy.savetxt("temp.txt", x, fmt='%f', delimiter=' ',newline='\r\n')
            self.OnShow()
            self.OnSuccess()
        ...
        def deleteButtonhelp(self,event):
            self.statusbar.SetStatusText("删除上一次输入的杆件信息")
